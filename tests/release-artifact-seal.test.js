...
test("release artifact seal accepts only the exact v1.1.10 fixture", (t) => {
  const fixture = createFixture(t);
  const result = runSeal(fixture.root);
  assert.equal(result.status, 0, result.stderr || result.stdout);
  assert.match(result.stdout, /"releaseTag": "v1\.1\.10"/);

  const wrongTag = runSeal(fixture.root, "v1.1.3");
  assert.notEqual(wrongTag.status, 0);
  assert.match(wrongTag.stderr, /release tag must be exactly v1.1.10/);
});
