...
test("v1.1.8 strict config accepts CC17 fade and rejects v1.1.7/legacy sequences", () => {
  const fs = require("node:fs");
  const v118 = JSON.parse(fs.readFileSync(require("node:path").join(__dirname, "..", "config", "dj-agent-v1.1.8.example.json"), "utf8"));
  assert.equal(validateFilterThenFadeThenStopShowConfig(v118, { allowTokenPlaceholder: true }), true);
  const retired = structuredClone(v118);
  ...
  const legacy = structuredClone(v118);
