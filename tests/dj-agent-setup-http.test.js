...
test("DJ Agent setup snapshot is localhost-only, token-free, and macro-safe", async (t) => {
  ...
  assert.equal(local.body.configTemplate.midi.releaseMacro.sequence, "filter-then-stop");
  assert.equal(local.body.mappingArtifact.valid, true);
  assert.equal(local.body.mappingArtifact.url, "/setup/CustomMIDI1-Syndocal-v1.1.7.csv");
