...
test("build-dist pins the exact Inno Setup compiler provenance order and rejects spoofable discovery", () => {
  ...
  assert.equal(packageJson.version, "1.1.8", "product package version must be the release version");
  ...
  assert.match(installerSource, /^AppVersion=1\.1\.8$/m, "installer AppVersion must match the corrective product version");
