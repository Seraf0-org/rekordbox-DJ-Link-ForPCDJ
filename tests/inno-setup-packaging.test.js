...
test("build-dist pins the exact Inno Setup compiler provenance order and rejects spoofable discovery", () => {
  ...
  assert.equal(packageJson.version, "1.1.6", "product package version must be the corrective release version");
  ...
  assert.match(installerSource, /^AppVersion=1\.1\.6$/m, "installer AppVersion must match the corrective product version");
