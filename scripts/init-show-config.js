...
const TEMPLATE_PATH = path.join(PROJECT_ROOT, "config", "dj-agent-v1.1.9.example.json");
const TARGET_PATH = String.raw`C:\SyndocalShow\dj-agent-v1.1.9.json`;
...
function parseAndValidateTemplate(raw) {
  ...
  if (!valid) {
    throw new ShowConfigInitializationError(
      ...
      "bundled show config template does not match the strict v1.1.9 filter-then-fade-then-stop contract",
    );
  }
  return config;
}
